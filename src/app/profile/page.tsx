  ...
  const handleAddReview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setReviewError(errJson.error || 'Не удалось отправить отзыв');
      }
    } catch (err: any) {
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in py-2">
      {saveSuccess && (
        <div className="px-4 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-mono text-xs flex items-center justify-between animate-in fade-in">
          ...
          <span className="text-[10px] text-zinc-500">Синхронизировано с сетью</span>
        </div>
      )}
            ...
            <div className="space-y-1.5">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="font-mono text-[10px] px-2.5 py-0.5 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20 font-semibold">
                  {isOwnProfile ? 'Личный кабинет' : 'Профиль инженера'}
                </span>
                {profileData?.role && (
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs font-mono text-zinc-400">
                <span className="flex items-center gap-1 text-amber-400 font-bold">
                  ...
                  <span className="text-zinc-500 font-normal">
                    ({stats.reviewsCount || reviews.length} отзывов)
                  </span>
                </span>
                <span>•</span>
        ...
        {isEditing ? (
          <form onSubmit={handleSaveProfile} className="space-y-4 pt-4 border-t border-white/[0.06]">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-mono text-zinc-400 mb-1.5">Локация</label>
                <input
                  type="text"
                  value={location}
                  onChange={(e) => setLocation(e.target.value)}
                  placeholder="например: Москва, Алматы, Remote"
                  className="w-full bg-[#18181f] border border-white/[0.08] focus:border-blue-500 rounded-xl px-3 py-2 text-xs text-white outline-none font-mono"
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-mono text-zinc-400 mb-1.5">
                О себе (стек, опыт, цели)
              </label>
              <textarea
                rows={3}
                value={bio}
                onChange={(e) => setBio(e.target.value)}
                placeholder="Разработка микросервисов, системное программирование, WebRTC, AI..."
                className="w-full bg-[#18181f] border border-white/[0.08] focus:border-blue-500 rounded-xl px-3 py-2 text-xs text-white outline-none font-mono leading-relaxed"
              />
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button
                ...
                className="px-4 py-2 rounded-xl bg-zinc-800 text-zinc-300 font-mono text-xs hover:bg-zinc-700 transition-colors"
              >
                Отмена
              </button>
              <button
          ...
          <p className="text-xs sm:text-sm text-zinc-300 leading-relaxed font-sans pt-2 border-t border-white/[0.06]">
            {p?.bio ||
              'Инженер платформы pixelmink. Готов к бартеру техническими знаниями и парному программированию.'}
          </p>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4 border-t border-white/[0.06] font-mono">
          ...
          <div className="p-4 bg-black/40 rounded-xl border border-white/[0.06] space-y-1">
            <div className="flex items-center justify-between text-zinc-500">
              <span className="text-[10px] uppercase font-bold tracking-wider">Созвонов</span>
              <PhoneCall className="w-3.5 h-3.5 text-blue-400" />
            </div>
            <div className="text-2xl font-bold text-white tracking-tight">{stats.callsCount}</div>
            <div className="text-[10px] text-zinc-500">проведено созвонов</div>
          </div>

          {/* 2. Часов на сайте / в сессиях */}
          <div className="p-4 bg-black/40 rounded-xl border border-white/[0.06] space-y-1">
            <div className="flex items-center justify-between text-zinc-500">
              <span className="text-[10px] uppercase font-bold tracking-wider">Часов в сессиях</span>
              <Clock className="w-3.5 h-3.5 text-emerald-400" />
            </div>
            <div className="text-2xl font-bold text-emerald-400 tracking-tight">
              {stats.totalHours} <span className="text-xs font-normal text-zinc-500">ч</span>
            </div>
            <div className="text-[10px] text-zinc-500">в сессиях и звонках</div>
          </div>

          {/* 3. Обучено коллег */}
          <div className="p-4 bg-black/40 rounded-xl border border-white/[0.06] space-y-1">
            <div className="flex items-center justify-between text-zinc-500">
              <span className="text-[10px] uppercase font-bold tracking-wider">Обучено коллег</span>
              <Award className="w-3.5 h-3.5 text-amber-400" />
            </div>
            <div className="text-2xl font-bold text-white tracking-tight">{stats.peersTaught}</div>
      ...
      {/* PEER REVIEWS & COMMENTS SECTION */}
      <div className="drinkit-card p-6 md:p-8 space-y-6">
        <div className="flex items-center justify-between pb-4 border-b border-white/[0.06]">
          <div>
            <h2 className="text-base font-bold text-white tracking-tight flex items-center gap-2">
              <MessageSquare className="w-4 h-4 text-blue-400" />
              Комментарии и отзывы коллег
            </h2>
        ...
        {!isOwnProfile && (
          <form
            ...
            className="p-5 rounded-2xl bg-[#0c0c10] border border-white/[0.08] space-y-4"
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-white font-mono flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-blue-400" />
                Оставить комментарий коллеге
              </span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs font-mono">
              <div className="p-3 rounded-xl bg-white/[0.02] border border-white/[0.04] flex items-center justify-between">
                <span className="text-zinc-400">Ясность объяснения:</span>
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((val) => (
                    <button
                      type="button"
              ...
              <div className="p-3 rounded-xl bg-white/[0.02] border border-white/[0.04] flex items-center justify-between">
                <span className="text-zinc-400">Понимание темы:</span>
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((val) => (
                    <button
                      type="button"
            ...
            <label className="flex items-center gap-2.5 cursor-pointer text-xs font-mono text-zinc-300 select-none">
              ...
              />
              <span>Рекомендую этого инженера для будущих созвонов и обмена</span>
            </label>
            ...
            <div>
              <textarea
                ...
                placeholder="Напишите отзыв: как прошел созвон, что полезного разобрали, насколько понятно коллега объяснял материал..."
            ...
            {reviewSuccess && (
              <div className="text-xs font-mono text-emerald-400 bg-emerald-500/10 p-2.5 rounded-lg border border-emerald-500/20 flex items-center gap-2">
                ...
                <span>Ваш отзыв успешно опубликован!</span>
              </div>
            )}
            ...
            <div className="flex justify-end">
              <button
                ...
                className="px-5 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-mono text-xs font-bold transition-all shadow-md shadow-blue-600/20 flex items-center gap-2"
              >
                {submittingReview ? (
        ...
        {isOwnProfile && (
          <div className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.06] flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs font-mono">
            ...
            <Link
              href="/matches"
              className="px-4 py-1.5 rounded-xl bg-[#18181f] hover:bg-zinc-800 text-blue-400 border border-blue-500/20 text-xs shrink-0 text-center transition-colors"
            >
              Найти коллегу в каталоге →
            </Link>
          </div>
        )}
          ...
          {reviews.length === 0 ? (
            ...
            reviews.map((r: any) => (
              ...
              >
                ...
                <p className="text-xs text-zinc-200 leading-relaxed font-sans">{r.comment}</p>
                ...
                <div className="flex flex-wrap items-center gap-3 text-[10px] font-mono text-zinc-500 pt-1 border-t border-white/[0.04]">
                  <span className="px-2 py-0.5 rounded bg-white/[0.02]">
                    Ясность: {r.clarityScore ?? 5}/5
                  </span>
                  <span className="px-2 py-0.5 rounded bg-white/[0.02]">
                    Глубина: {r.understandScore ?? 5}/5
                  </span>
                  {r.wouldStudyAgain && (
                    <span className="text-emerald-400 flex items-center gap-1">
                      <ThumbsUp className="w-3 h-3" />
                      <span>Рекомендует коллегу</span>
                    </span>
                  )}
                </div>
              </div>
  ...
  return (
    <Suspense
      fallback={
        <div className="max-w-4xl mx-auto py-16 flex flex-col items-center justify-center space-y-4">
          <Loader2 className="w-8 h-8 text-blue-500 animate-spin" />
          <span className="font-mono text-xs text-zinc-400">Загрузка профиля...</span>
        </div>
      }
    >
      <ProfileContent />
